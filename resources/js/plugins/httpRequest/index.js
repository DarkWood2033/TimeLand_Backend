...
export default ({routes, config}) => {
    ...
    function send(query, driver){
        // Отправка запроса и обработка ответа
        return config.drivers[driver || 'default'](query);
    }

    return {
        route: function (name, {segments, driver, data, params, headers} = true) {
            //  Получение данных о роуте
            let route = routing(routes, name, segments);
            ...
            // Отправка запроса
            return send(route, driver);
        },

        query: function (query, driver) {
            // Отправка запроса
            return send(query, driver);
        }
    };
};
